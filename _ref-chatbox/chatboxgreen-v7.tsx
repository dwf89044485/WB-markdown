...
export type ChatboxGreenV7Props = ComposerCoreOptions;

export function ChatboxGreenV7(props: ChatboxGreenV7Props = {}) {
  ...
  // ── Layout measurement (variant-specific) ──
  ...
  useLayoutEffect(() => {
    ...
    const updateCompactHeight = () => {
      ...
      setCompactHeightPx(Math.max(72, measured));
    };
    updateCompactHeight();
  ...
  // ── Backdrop (variant-specific visual behavior) ──
  ...
  // 图片推荐蒙层：有图片附件 + 推荐项可见 + 胶囊推荐项未激活（避免两者重叠）
  const showImageSuggestionsBackdrop = core.expanded && showImageSuggestions && !core.hasRecommendations;
  ...
  useEffect(() => {
    if (core.frozen) { clearBackdrop("v7-input"); clearBackdrop("v7-holding"); return; }
    if (showInputBackdrop) {
      requestBackdrop({ id: "v7-input", visible: true, onDismiss: voice.voiceEditMode ? undefined : handleInputBackdropDismiss, animationDuration: 200 });
    } else { clearBackdrop("v7-input"); }
  }, [showInputBackdrop, core.frozen, requestBackdrop, clearBackdrop, handleInputBackdropDismiss, voice.voiceEditMode]);

  useEffect(() => {
    if (core.frozen) { clearBackdrop("v7-holding"); return; }
    if (showHoldingBackdrop) {
      requestBackdrop({ id: "v7-holding", visible: true, animationDuration: 600 });
    } else { clearBackdrop("v7-holding"); }
  }, [showHoldingBackdrop, core.frozen, requestBackdrop, clearBackdrop]);

  useEffect(() => {
    if (core.frozen) { clearBackdrop("v7-image-attach"); return; }
    if (showImageSuggestionsBackdrop) {
      requestBackdrop({ id: "v7-image-attach", visible: true, onDismiss: () => setShowImageSuggestions(false), animationDuration: 200 });
    } else { clearBackdrop("v7-image-attach"); }
  }, [showImageSuggestionsBackdrop, core.frozen, requestBackdrop, clearBackdrop]);

  useEffect(() => () => { clearBackdrop("v7-input"); clearBackdrop("v7-holding"); clearBackdrop("v7-image-attach"); clearBackdrop("slash-skills"); }, [clearBackdrop]);
  ...
  useSlashTrigger({
    ...
    onTrigger: () => {
      setPanelOpen(true);
      // backdrop 互斥：让 slash-skills 唯一显示
      clearBackdrop("v7-input");
      setShowImageSuggestions(false);
    },
    onSlashCleared: () => {
      setPanelOpen(false);
  ...
  // ── Submit (orchestrates core + voice) ──
  // source='sendButton' 表示由可见发送按钮本身触发：避免再次 click sendButtonRef 导致递归
  // ComposerPrimitive.Send 内置 onClick 会通过 Radix Slot 合并执行真正的发送，无需我们再 click
  ...
  return (
    ...
    <ComposerPrimitive.Root className={cn("relative w-full px-[26px] z-50", core.frozen && "pointer-events-none")}>
      ...
      {/* Fullscreen dimmer */}
      <div
        className="pointer-events-none fixed inset-x-0 top-0 z-0 bg-black transition-opacity duration-300 ease-out"
      ...
      {/* Image attachment suggestions above input */}
      ...
      >
        {core.expanded ? (
          core.fullScreen ? (
            <FullScreenLayout core={core} voice={voice} submitComposer={submitComposer} isComposingRef={isComposingRef} />
          ) : (
          ...
          <CollapsedLayout core={core} voice={voice} />
        )}
      </motion.div>
      ...
      {!core.expanded && (core.voiceMode || voice.voiceOverlayOpen) && (
        <div className="pointer-events-none absolute h-px w-px overflow-hidden opacity-0" aria-hidden>
          <ComposerPrimitive.Input
            ...
            tabIndex={-1}
          />
          <SendButton buttonRef={core.sendButtonRef} hasContent={voice.voiceEditText.trim().length > 0} onSubmit={() => submitComposer("sendButton")} />
        </div>
      )}

      {/* Virtual keyboard */}
      <VirtualKeyboardPanel
        open={voice.voiceEditMode ? voice.voiceEditKeyboardOpen : core.expanded && core.hasKeyboard && !core.voiceMode && !voice.holding}
        ...
        onPanelPointerDown={voice.voiceEditMode ? () => { voice.voiceEditTextareaRef.current?.focus({ preventScroll: true }); } : core.keepComposerFocused}
...
// ─────────────────────────────────────────────────────────────────
// Sub-components (variant-specific layout)
// ─────────────────────────────────────────────────────────────────

function FullScreenLayout({ core, voice, submitComposer, isComposingRef }: { core: ReturnType<typeof useComposerCore>; voice: ReturnType<typeof useComposerVoiceEdit>; submitComposer: (source?: "sendButton" | "other") => void; isComposingRef: React.RefObject<boolean> }) {
  return (
    <div className="relative flex h-full w-full flex-col px-[20px] pt-[10px] pb-[20px]">
      ...
      <div className="pointer-events-none absolute right-[20px] bottom-[20px]">
        <div className="pointer-events-auto relative h-[26px] w-[26px] shrink-0">
          <SendButton buttonRef={core.sendButtonRef} hasContent={core.hasComposerContent} onSubmit={() => submitComposer("sendButton")} />
        </div>
      </div>
    </div>
  );
...
function CompactExpandedLayout({ core, voice, submitComposer, compactContentRef, ChipIcon, onZapClick, isComposingRef }: {
  core: ReturnType<typeof useComposerCore>;
  voice: ReturnType<typeof useComposerVoiceEdit>;
  ...
  return (
    <div ref={compactContentRef} className="flex w-full flex-col items-start p-[10px]">
      ...
      <div className="flex w-full flex-col px-[4px] py-px">
        ...
        {core.chip && (
          <button type="button" tabIndex={-1}
            onPointerDown={(e) => {
              // 仅阻止默认行为（防止 textarea 失焦）和事件冒泡
              // 不在此处修改任何状态：本按钮点击会 unmount 自己，
              // 若在 pointerdown 同步 unmount，pointerup 会落到下方元素（首页胶囊），
              // 触发"深度研究/视频生成"的 dispatchChip
              e.preventDefault();
              e.stopPropagation();
            }}
            onClick={(e) => {
              e.stopPropagation();
              core.suppressEmptyCollapseRef.current = true;
              core.clearChip();
            }}
      ...
      {voice.voiceEditMode ? (
        <V7VoiceEditToolbar
          holding={voice.holding}
          onClear={voice.discardVoiceEdit}
          onSubmit={submitComposer}
          onHoldPointerDown={(e) => {
            e.preventDefault();
...
function CollapsedLayout({ core, voice }: { core: ReturnType<typeof useComposerCore>; voice: ReturnType<typeof useComposerVoiceEdit> }) {
  return (
...
function V7VoiceEditToolbar({ holding, onClear, onSubmit, onHoldPointerDown, children }: {
  holding: boolean; onClear: () => void; onSubmit: () => void;
  onHoldPointerDown: (e: React.PointerEvent<HTMLButtonElement>) => void; children: React.ReactNode;
}) {
  const pillStyle: React.CSSProperties = {
    height: "34px", borderRadius: "999px", padding: "0 12px",
    background: "rgba(255,255,255,0.38)", color: "rgba(0,0,0,0.62)",
    fontSize: "12px", lineHeight: "16px",
    WebkitUserSelect: "none", userSelect: "none", WebkitTouchCallout: "none",
  };
  return (
    <div className="relative flex w-full items-end justify-between pt-[2px] select-none" style={{ WebkitUserSelect: "none", touchAction: "none" }}>
      <button type="button" tabIndex={-1} className="shrink-0" style={pillStyle}
        onPointerDown={(e) => { e.preventDefault(); onClear(); }} draggable={false}>清空并返回</button>
      <div className="flex flex-col items-center" style={{ gap: "6px" }}>
        <span style={{ fontSize: "10px", lineHeight: "14px", color: "rgba(0,0,0,0.5)" }}>长按说话</span>
        <button type="button" tabIndex={-1} aria-label="长按说话"
          className="flex size-[42px] items-center justify-center rounded-full active:scale-95 transition-transform"
          style={{ background: holding ? "rgba(255,255,255,0.82)" : "rgba(255,255,255,0.42)", boxShadow: "0 2px 10px rgba(0,0,0,0.08)", WebkitTouchCallout: "none" }}
          onPointerDown={onHoldPointerDown} onContextMenu={(e) => e.preventDefault()} draggable={false}>
          <img src="/icons/mic-button.svg" alt="" width={22} height={22} draggable={false} />
        </button>
      </div>
      <button type="button" tabIndex={-1} className="shrink-0" style={{ ...pillStyle, color: "rgba(0,0,0,0.86)" }}
        onPointerDown={(e) => { e.preventDefault(); onSubmit(); }} draggable={false}>发送消息</button>
      <div className="pointer-events-none absolute h-px w-px overflow-hidden opacity-0">{children}</div>
    </div>
  );
}

function SendButton({ buttonRef, hasContent, onSubmit }: { buttonRef: React.RefObject<HTMLButtonElement | null>; hasContent: boolean; onSubmit?: () => void }) {
  return (
