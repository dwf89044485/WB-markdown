...
每次新增或改造 Feature Panel 页面前，先用这张表检查。只有通过检查的内容，才应该进入页面或升格为 spine 规则。

| 检查项 | 合格标准 | 不合格信号 |
|---|---|---|
| 对象边界 | 页面在讲“如何帮助读者理解 Agent 设计” | 页面开始重新定义 Agent 产品本身应该如何工作 |
| 页面类型 | 明确属于决策层、规范层、执行层，或说明为什么混合 | 读者不知道这页是在讲原则、结构还是功能细节 |
| 读者场景 | 开头能说明谁会看、为什么看 | 直接进入控件细节 |
| 信息顺序 | 先判断，再结构，再细节，最后示例验证 | 上来就讲按钮、参数、样式 |
| 图文表达 | 状态、流程、构成有表格、流程图、快照或对比图 | 复杂行为只用纯文字描述 |
| 示例关系 | 示例按钮说明“将看到什么” | 按钮只写“查看”“跳转”“运行” |
| 工程边界 | 只讲交互逻辑和产品规则 | 写入 JS/CSS 文件路径、函数名、事件绑定等实现逻辑 |
| Spine 归属 | 视觉外观进入 `DESIGN.md`，阅读路径和状态规则进入 `EXPERIENCE.md` | 把视觉 token 和体验流程混在同一段里 |
| Open Questions | 未确认内容明确暴露 | AI 推断被写成确定规则 |
