  ...
  get content() {
    ...
    <article class="fp-feature">
      ...
      <section data-section="interaction">
        ...
        <h3>4.2 决策语义与后续流向</h3>
        <p>三个选项不是"多选一"，而是<strong>授权范围的梯度</strong>。每个选项都有明确的产品语义：</p>
        <table>
          <thead>
            <tr><th>选项</th><th>语义</th><th>授权范围</th><th>Agent 后续流向</th></tr>
          </thead>
          <tbody>
            <tr><td>允许</td><td>单次放行</td><td>仅这一次操作</td><td>继续执行当前操作；下次同类操作仍需询问</td></tr>
            <tr><td>本次会话内始终允许</td><td>会话级信任</td><td>本次会话内所有同类命令</td><td>继续执行；后续同类操作自动放行，不再打扰</td></tr>
            <tr><td>拒绝</td><td>终止该操作</td><td>无</td><td>放弃当前操作方案，Agent 需换路径或停止该动作</td></tr>
          </tbody>
        </table>
        <blockquote>
          <p><strong>Demo 实现口径</strong>：当前 Demo 剧本是线性播放的，三个选项都会让播放继续（选择结果记录到 console，不分支剧情）。上表描述的是产品语义层的约定——文档定义"应该怎样"，Demo 演示"看起来怎样"。</p>
        </blockquote>
      ...
      <section data-section="rationale">
        ...
        <h3>为什么是内嵌在输入栏位置，而不是模态弹层</h3>
        <p>批准权限发生时，对话流里刚刚展示了 Agent 要做什么、为什么——这些信息正是用户做决策的依据。模态弹层会把这些上下文遮住，逼用户凭记忆决策。内嵌在输入栏位置（对话流末尾、用户视线自然落点）则相反：<strong>不遮罩、不打断空间、上下文全部可见</strong>，只是轻轻地卡在必经之路上。它占用输入栏位置还有一层含义：此刻用户唯一能"输入"的，就是这个审批答案。</p>
