  ...
  content: `
    <article class="fp-feature">
      ...
      <section data-section="modes">
        <h2>2. 设计样式</h2>
        <p>视觉密度 3 档。下方 3 块用 3 条任务「搜索网页 / 创建文件 / 读取文件」、相同动效时长循环演示「运行中 → 完成」全过程；<strong>stack 模式在完成时折叠为「图标堆 + 已执行 3 项」</strong>。</p>
        <div class="fp-snapshot-grid-3 fp-tcn-modes">
          <div class="fp-snapshot-wrap">
            <span class="tag">card · 卡片</span>
            <div class="fp-snapshot">${modeBlock('card', 'tool-call-card', statusLineHTML(M_RUN))}</div>
          </div>
          <div class="fp-snapshot-wrap">
            <span class="tag">flat · 内联</span>
            <div class="fp-snapshot">${modeBlock('flat', 'tool-call-flat', statusLineHTML(M_RUN))}</div>
          </div>
          <div class="fp-snapshot-wrap">
            <span class="tag">stack · 折叠</span>
            <div class="fp-snapshot">${modeBlock('stack', 'tool-call-stack', statusLineHTML(M_RUN))}</div>
          </div>
        </div>
      </section>
