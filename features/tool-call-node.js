  ...
  content: `
    <article class="fp-feature">
      ...
      <section data-section="anatomy">
        <h2>2. 构成（结构）</h2>
        <p>工具调用节点是一条可点击的状态行，位于 AI 消息气泡内、思考按钮下方。进行中的时候，整行带扫光动画；完成后扫光消失，文字颜色变浅。</p>
        <div class="fp-snapshot-wrap">
          <span class="tag">进行中（单条工具）</span>
          <div class="fp-tcn-inline">${sl([L.run], { state: 'running' })}</div>
        </div>
        <div class="fp-anatomy-list">
          <div class="fp-anatomy-item">
            <span class="fp-anatomy-num">①</span>
            <div>
              <h4>工具图标</h4>
              <p>根据工具类型自动推断（搜索 → 搜索图标、执行命令 → 终端图标）。失败事件显示警告图标。</p>
            </div>
          </div>
          <div class="fp-anatomy-item">
            <span class="fp-anatomy-num">②</span>
            <div>
              <h4>文字描述</h4>
              <p>进行中前缀「正在」，完成后只去掉「正在」（不加「已」）。同一段文案，两个状态共用，只靠前缀区分。</p>
            </div>
          </div>
          <div class="fp-anatomy-item">
            <span class="fp-anatomy-num">③</span>
            <div>
              <h4>扫光动画（仅进行中）</h4>
              <p>一条光从左向右 1.15s 循环扫过文字，表示 Agent 还在工作。只有最末那条（进行中）带扫光，已完成的条目不带。</p>
            </div>
          </div>
          <div class="fp-anatomy-item">
            <span class="fp-anatomy-num">④</span>
            <div>
              <h4>展开箭头 ›</h4>
              <p>点击整行弹出 Sheet 查看执行详情。进行中时箭头深色，完成后变浅，表示「可查看但不再活跃」。</p>
            </div>
          </div>
        </div>
      </section>
